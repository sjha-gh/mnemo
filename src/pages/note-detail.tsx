...
import { ArrowLeft, Pencil, Play, Pause, Sparkles, Tag, Wand2 } from "lucide-react"
...
export function NoteDetailPage() {
  ...
  // Auto-run the AI pipeline once for notes that were just saved (queued, no summary yet).
  useEffect(() => {
    if (!note || startedRef.current) return
    if (note.status === "queued" && !note.aiSummary) runIndex()
  ...
  const isIndexing = index.isPending || status === "processing" || status === "queued"
    ...
    <article className="mx-auto flex w-full max-w-2xl flex-col gap-6">
      ...
      <header className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">Updated {relativeTime(note.updatedAt)}</span>
        </div>
      ...
      <section className="rounded-2xl border border-primary/20 bg-primary/[0.04] p-4">
        ...
        {isIndexing ? (
          <p className="mt-3 text-sm text-muted-foreground">
            {status === "queued" ? "Queued for indexing…" : "Analyzing your note…"}
          </p>
        ) : note.aiSummary ? (
          <div className="mt-3 flex flex-col gap-3">
            <p className="text-sm leading-relaxed text-foreground">{note.aiSummary}</p>
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            This note hasn&apos;t been indexed yet. Run indexing to generate a summary and keywords.
          </p>
        )}
      </section>
            ...
            {note.images.map((img) => (
              <figure key={img.id} className="flex flex-col gap-1.5">
                <div className="aspect-square overflow-hidden rounded-xl border border-border bg-muted">
                  <img src={img.url || "/placeholder.svg"} alt={img.alt} className="size-full object-cover" />
                </div>
                {img.caption && (
      ...
      {note.audioClips.length > 0 && (
        <section className="flex flex-col gap-3">
          <h2 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Voice memos</h2>
